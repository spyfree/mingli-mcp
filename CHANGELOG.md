...
- **晚子时换日改走子初换日**: `EightChar` 默认 `sect=2`（夜子时：日柱留在当日、
  时柱取次日子时），官网走的是子初换日——23:00 起日柱即进位次日。实测
  2024-01-15 23:00 出生，本服务给 `日柱戊寅`、官网给 `日柱己卯`，**日主不同，
  十神、格局、大运全不同**。现固定 `setSect(1)` 与官网一致。这是流派选择不是
  对错，但两端必须相同。**受影响的是 23:00-23:59 出生的用户，其日柱与旧版本不同。**
- **时辰代表小时改取中点**: 原实现取时辰起点（`(time_index-1)*2+1`，午时=11:00），
  官网取中点（午时=12:00）。平时没差别，交节当天会现形：某个节若交在 11:30，
  起点归上一个月建、中点归下一个月建，两端排出不同月柱。

### 新增

- **跨引擎金标向量** `docs/cross-engine-vectors.json`：与官网仓库各存一份、内容
  逐字一致，写明时辰映射、交节换柱、晚子时换日各走哪一派，并给出 12 组必须一致的
  四柱向量。`tests/test_cross_engine_vectors.py` 逐条断言。**任何一端要改口径，
  先改这张表并两边同步，再改代码。**

### 依赖

- `iztro-py` 钉到 `>=0.5.0,<0.6`。原先写的是 `>=0.4.0`，没有上界，而 0.5.0 带了
  一条破坏性变更（`feat(i18n)!: honour language on top-level fields`）和一条
  `fix(calendar): correct get_day_stem_branch`——下一次容器重建会静默吃进去。
  已在 0.5.0 下跑通全量测试（416 passed）再钉。
